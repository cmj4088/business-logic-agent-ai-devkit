...
def create_app() -> FastAPI:
    ...
    # === 生产模式：自托管前端静态文件 ===
    # 检测 frontend/dist 目录，存在则挂载为静态文件服务
    # 这样只需启动后端一个进程，即可同时提供 API 和前端页面
...
def _mount_frontend_static(app: FastAPI) -> None:
    ...
    检测 frontend/dist 目录是否存在，存在则挂载为静态文件服务。
    支持 SPA 路由（所有未匹配路径回退到 index.html）。
    """
    ...
    logger = logging.getLogger("bla.frontend")

    class SPAStaticFiles(StaticFiles):
        """SPA 友好的静态文件服务：404 时回退到 index.html。"""
        async def get_response(self, path: str, scope):
            try:
                response = await super().get_response(path, scope)
                if response.status_code == 404:
                    # SPA 路由回退（如直接访问 /projects/xxx）
    ...
    logger.info("✅ 前端静态文件已挂载（%s），访问 http://localhost:8000 使用应用", frontend_dist)
